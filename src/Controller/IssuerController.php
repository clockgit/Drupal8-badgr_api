...
class IssuerController extends ControllerBase implements ContainerInjectionInterface {
   ...
   *
   * @param \Drupal\badgr\Entity\BadgrEntityInterface $issuer
   *   A Issuer  object.
   *
   * @return array
   *   An array as expected by drupal_render().
   */
  public function revisionOverview(BadgrEntityInterface $issuer) {
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              'title' => $this->t('Revert'),
              'url' => Url::fromRoute('entity.issuer.revision_revert', ['issuer' => $issuer->id(), 'issuer_revision' => $vid]),
            ];
          }
