...
class AssertionController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionShow($assertion_revision) {
    $assertion = $this->entityManager()->getStorage('assertion')->loadRevision($assertion_revision);
   ...
   *
   * @param \Drupal\badgr\Entity\BadgrEntityInterface $assertion
   *   A Assertion  object.
   *
   * @return array
   *   An array as expected by drupal_render().
   */
  public function revisionOverview(BadgrEntityInterface $assertion) {
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              'title' => $this->t('Revert'),
              'url' => Url::fromRoute('entity.assertion.revision_revert', ['assertion' => $assertion->id(), 'assertion_revision' => $vid]),
            ];
          }
          ...
          if ($delete_permission) {
            $links['delete'] = [
              'title' => $this->t('Delete'),
              'url' => Url::fromRoute('entity.assertion.revision_delete', ['assertion' => $assertion->id(), 'assertion_revision' => $vid]),
            ];
          }
